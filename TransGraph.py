...
from trans_model import TransformsConvGraph
...
for epoch in range(num_epoch):
    ...
    if ret[1]<best_mse:
        ...
        best_mse = ret[1]
        best_pearson = ret[2]
        print(' rmse improved at epoch ', best_epoch, '; best_mse:', best_mse, model_st,'GDSC')
    else:
...
def draw_loss(train_losses, test_losses, title):
    ...
    plt.legend()
    # save image
    plt.savefig('/mnt/double-graph/pictures/' + title+".png")  # should before show method
...
def draw_pearson(pearsons, title):
    ...
    plt.legend()
    # save image
    plt.savefig('/mnt/double-graph' + title+".png")  # should before show method

draw_loss(train_losses, val_losses, 'loss_transformer')
draw_pearson(val_pearsons, 'pearson_transformer')