...
# TransformConv based model
class TransformsConvGraph(torch.nn.Module):
    def __init__(self, n_output=1, n_filters=32, embed_dim=128,num_features_xd=78, num_features_xt=25, output_dim=128, dropout=0.5):

        super(TransformsConvGraph, self).__init__()

        # SMILES graph branch
        self.n_output = n_output
        self.conv1 = TransformerConv(num_features_xd, num_features_xd)
        self.conv2 = TransformerConv(num_features_xd, num_features_xd*2)
        self.conv3 = TransformerConv(num_features_xd*2, num_features_xd * 4)
